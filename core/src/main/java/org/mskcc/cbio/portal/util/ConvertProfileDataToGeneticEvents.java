...
public class ConvertProfileDataToGeneticEvents {
   ...
   public static GeneticEvent[][] convert(ProfileDataSummary pSummaryData, String[] geneList, 
            OncoPrintSpecification theOncoPrintSpecification, double mRnaZscoreUpThreshold, double mRnaZscoreDownThreshold, double proteinExpZscoreUpThreshold, double proteinExpZscoreDownThreshold) {
      ...
      for( String currentGene : goodGenes ){
         ...
         for (int j = 0; j < pData.getCaseIdList().size(); j++) {
            ...
            ValueParser valueParser = ValueParser.generateValueParser( 
                currentGene, value, mRnaZscoreUpThreshold, mRnaZscoreDownThreshold, proteinExpZscoreUpThreshold, proteinExpZscoreDownThreshold, theOncoPrintSpecification );
            if( null == valueParser){
