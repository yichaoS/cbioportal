      ...
      '<span class="breadcrumb_container" v-if="attributes.filter.length > 0">' +
        ...
        '<span v-if="attributes.attr_id !== \'MUT_CNT_VS_CNA\'" class="breadcrumb_items">' +
          '<span v-if="filters.filterType === \'RangedFilter\'">' +
            ...
            '<img class="breadcrumb_remove" src="images/dashboard/remove_breadcrumb_icon.png" @click="removeFilter(filters)">' +
          '</span>' +
          '<span v-else>' +
            '<span v-for="filter in filters" style="display:inline-block;">' +
              '<span class="breadcrumb_item">{{filter}}</span>' +
              '<img class="breadcrumb_remove" src="images/dashboard/remove_breadcrumb_icon.png" @click="removeFilter(filter)">' +
            '</span>' +
          '</span>' +
        '</span>' +
        '<span v-else>' +
          '<img class="breadcrumb_remove" src="images/dashboard/remove_breadcrumb_icon.png" @click="removeFilter()">' +
        '</span>' +
      '</span>',
    props: [
      'filters', 'attributes'
